...
Full write-up: [`docs/latex/main_extended.pdf`](docs/latex/main_extended.pdf) (5 pages: body pp. 1–4 + references).
...
Raw datasets, embeddings, and trained models are **gitignored** and regenerated locally; the IU X-Ray/RadLex CSVs and all `results/` (138 tracked files) are committed.
...
| SAE training (5 seeds) | `python scripts/run_sae_training.py` | `models/sae_seed*/`, `results/training_manifest.json` |
...
docs/latex/           paper source + main_extended.pdf
docs/literature/      reference papers (PDFs gitignored)
docs/presentation/    slide deck (deck.pdf, index.html) + assets
results/              experiment outputs + REPORT.md per folder (tracked)
```
