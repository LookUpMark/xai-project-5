...
All experiments use \emph{two} chest-radiography corpora. IU X-Ray~\cite{demner2016iuxray} (7,470 frontal/lateral images, 3,955 reports) is the primary, small-scale setting; ROCOv2 (${\sim}80$k images) is a $10{\times}$-scale replication that lets us test the data-volume hypothesis directly. For Gap~6 we split \emph{by radiographic study} (IU key $=\texttt{patient\_IM-study}$; ROCOv2 by image, no shared patient structure to leak), so no IU study appears in both partitions. The IU split yields 5,955 train / 1,515 test images (3,081 / 771 studies) with verified group overlap $=0$, recomputed deterministically on every run (sorted study set, fixed seed).
