...
\textbf{Limitations and future work.} The LLM-judge is implemented (verb$\to$label, $\mathrm{temperature}{=}0$, count-matched LIFT) but not yet run with MedGemma-4B~\cite{sellergren2025medgemma} to convergence, so method rankings are deferred (with bootstrap CIs). The decisive untested mitigation is the representation site: an SAE on the BiomedCLIP \emph{patch-token residual stream} at a mid layer (not the pooled CLS)~\cite{bricken2023monosemanticity}---the experiment our diagnosis predicts would restore identifiability.
