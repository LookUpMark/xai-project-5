...
\textbf{Limitations and future work.} The LLM-judge (Sec.~\ref{sec:judge}) scores high but model-dependent local plausibility, orthogonal to identifiability. The decisive mitigation is an SAE on the BiomedCLIP \emph{patch-token residual stream} (mid layer, not pooled CLS)~\cite{bricken2023monosemanticity}---the experiment our diagnosis predicts would restore identifiability.
