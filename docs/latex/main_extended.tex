...
\textbf{Medical VLMs and the reference framework.} BiomedCLIP~\cite{zhang2023biomedclip} is a domain-specific contrastive VLM (ViT-B/16 image encoder + PubMedBERT text encoder, pretrained on PMC-15M) whose image encoder exposes a 768-d pre-projection hidden state; its released checkpoint projects this into the 512-d shared contrastive space our pipeline decomposes. MedConcept~\cite{haque2026medconcept} provides the methodological template---sparse extraction, vocabulary alignment, and LLM-judge evaluation with the Aligned/Unaligned/Uncertain metrics---but operates on \emph{3D volumetric} abdominal CT with the Merlin~\cite{blankemeier2024merlin} vision-language foundation model, which is far more compute- and data-demanding than our 2D setting; we adapt its paradigm, not replicate its backbone. RadLex~\cite{langlotz2006radlex} and MeSH~\cite{nlm2026mesh} provide the external radiology lexicons used for naming and organisation.
...
We use BiomedCLIP (ViT-B/16, 768-d hidden state projected by a frozen $\mathrm{visual\_projection}: \mathbb{R}^{768}\!\to\!\mathbb{R}^{512}$ into a shared contrastive space; images at $224{\times}224$, embeddings L2-normalised). The shared 512-d space exhibits a \emph{modality gap}~\cite{liang2022mindthegap}; following prior work we estimate it as the difference of the mean image and vocabulary embeddings and subtract it before cosine naming/decomposition.
