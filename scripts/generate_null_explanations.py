"""generate_null_explanations.py — random-k chance-floor baseline for the LLM judge (F-005).
...
Usage:
    python scripts/generate_null_explanations.py
    python scripts/generate_null_explanations.py --k 18          # fixed count
"""
    ...
    if args.k is None:
        if not spliece_path.exists():
            raise FileNotFoundError(
                f"{spliece_path} missing; run `python scripts/run_spliece.py` first, "
