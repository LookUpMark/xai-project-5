...
def evaluate(
    resume: bool = False,
    batch_save_every: int = judge_cfg.batch_save_every,
    source: str = "baseline",
) -> Path:
    """
...
Examples:
  ...
  python src/evaluate_llm_judge.py --dataset iu_xray --source spliece --resume
    ...
    parser.add_argument(
        "--checkpoint-every",
        type=int,
        default=judge_cfg.batch_save_every,
        help="Save checkpoint every N evaluations (default: config.judge.batch_save_every)",
    )
    parser.add_argument(
        "--dataset",
        type=str,
