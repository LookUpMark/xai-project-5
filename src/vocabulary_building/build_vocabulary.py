...
from vocabulary_building.radlex_support import load_and_filter_radlex
...
def save_vocab_embeddings(
    ...
    Args:
        vocabulary: list of vocabulary entry dicts.
        terms: full list of CSV terms (same order as all_embeddings).
        all_embeddings: (N, 512) embeddings for all CSV terms.
        config (VocabularyConfig): dataclass containing parameters.
    """
    term_to_idx = {t.lower(): i for i, t in enumerate(terms)}
...
def build_vocabulary_pipeline(
    ...
    Full vocabulary building pipeline:
        1. Filter terms to the target domain via the RadLex hierarchy
           (radlex_support.load_and_filter_radlex)
        2. Inject missing NIH seed terms into the filtered list
        3. Encode all terms with BiomedCLIP text encoder
        4. Compute clinically defined macro-centroids (hard-clustering)
        5. Rank terms by max cosine similarity to any macro-centroid
        6. Select top-k input terms + NIH seed terms
        7. Save vocabulary JSON and pre-computed embeddings
    ...
    """
    # Filter terms to the chest-X-ray domain via the RadLex hierarchy BEFORE
    # encoding (cuts embedding cost + vocabulary noise). The filter lives in
    # radlex_support.py, reads the RadLex CSV once, and returns the
    # domain-relevant preferred labels (order-preserving).
    filtered = load_and_filter_radlex(vocab_config.input_csv_path)
    terms = list(filtered)

    input_terms_set = {t.lower() for t in terms}

    # Inject missing NIH seeds
    missing_seeds = [t for t in vocab_config.nih_seed_terms if t.lower() not in input_terms_set]
    if missing_seeds:
        terms.extend(missing_seeds)
    ...
    ranked_terms = rank_terms_by_relevance(terms, all_embeddings, anchor_centroids)
